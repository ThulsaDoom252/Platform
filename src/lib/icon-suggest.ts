...
const CONCEPT_ALIASES: [string, string][] = [
  ...
  // Действия и качества: связи строятся сразу по английскому и переводу,
  // поэтому literal make или случайное созвучие больше не перетягивает смысл.
  ["⏱️", "find time allocate time set aside time знаходити час виділяти час находить время выделять время"],
  ["✅", "ensure ascertain verify переконатися впевнитися убедиться удостовериться"],
  ["💪", "effort endeavour endeavor try hard докласти зусиль прикласти усилия стараться"],
  ["🤔", "decision decide choose choice приймати рішення ухвалити рішення принять решение выбор"],
  ["🛠️", "put right fix correct repair remedy виправити ситуацію исправить ситуацию починить"],
  ["🧠", "conscious consciousness subconscious subconsciousness concesness sub concsness sub-concsness свідомість підсвідомість сознание подсознание"],
  ["🤖", "artificial synthetic man made fake штучний искусственный синтетичний синтетический"],
  ["🚀", "cutting edge cutting-edge advanced leading edge state of the art передовий передовой новітній новейший"],
  ["👍", "decent respectable satisfactory proper good enough пристойний приличный достойный непоганий"],
  ["⚠️", "faulty defective malfunctioning broken несправний неисправный дефектний дефектный"],
  ["🥚", "fragile breakable delicate brittle крихкий хрупкий ламкий"],
  ["🧱", "rough coarse uneven scratchy tactile жорсткий грубий шершавый грубый неровный тактильно"],
  ["🧼", "smooth sleek even polished silky гладкий плавний гладко ровный"],
  ["🎩", "sophisticated refined cultured elegant intricate витончений изысканный складний сложный утонченный"],
  ["🐜", "trivial minor insignificant petty unimportant дрібязковий неважливий незначительный мелочный"],
  ["🤷", "not a big deal no big deal nothing serious no problem нічого страшного не проблема ничего страшного"],
  ["☝️", "heres the deal here is the deal listen conditions ось у чому справа слухай умови вот в чем дело условия"],
  // Здоровье и лечение. Используем старые, широко поддерживаемые emoji —
  // автоматический подбор не должен превращаться в пустой квадрат Windows.
  ["🦴", "backache back pain fracture broken bone біль у спині перелом боль в спине кость"],
  ["💧", "blister skin bubble пухир волдырь мозоль"],
  ["🤕", "bruise bruised wound injury синець рана ушиб синяк ранение"],
  ["🤧", "cold flu runny nose застуда грип нежить простуда насморк"],
  ["😷", "cough coughing кашель кашляти кашлять"],
  ["😫", "fatigue tiredness exhaustion втома усталость утомление"],
  ["😵", "faint fainting unconscious непритомність обморок потеря сознания"],
  ["🤒", "fever temperature symptom жар температура симптом ознака болезни"],
  ["🤢", "food poisoning nausea poisoned нудота харчове отруєння тошнота пищевое отравление"],
  ["🔴", "rash skin eruption висип сыпь раздражение кожи"],
  ["🦠", "stomach bug stomach infection infection шлункова інфекція кишечная инфекция"],
  ["☀️", "sunburn sun burn сонячний опік солнечный ожог"],
  ["🦷", "toothache tooth pain зубний біль зубная боль"],
  ["📝", "prescription medical prescription рецепт призначення назначение врача"],
  ["🏥", "medical examination general practitioner doctor gp surgery operation обстеження сімейний лікар операція обследование врач операция"],
  ["⛑️", "first aid kit first-aid kit аптечка первая помощь перша допомога"],
  ["🧴", "ointment cream salve мазь крем лікувальна мазь"],
  ["💊", "painkiller painkillers analgesic analgesics знеболювальні обезболивающие"],
  ["😴", "sedative sedatives tranquilizer заспокійливі успокоительные снотворное"],
  ["⚠️", "side effect adverse effect побічний ефект побочный эффект"],
  ["📡", "ultrasound ultrasonic diagnostic scan ультразвукова діагностика узи ультразвук"],
  ["🌱", "welfare wellbeing well being добробут благополучие благосостояние"],
  ["☢️", "x ray x-ray radiograph рентген рентгенівський снимок"],
];
...
const PHRASE_ICON_RULES: [RegExp, string][] = [
  ...
  [/\bmake an effort\b/u, "💪"],
  [/\bmake a decision\b/u, "🤔"],
  [/\bmake it right\b|\bput it right\b/u, "🛠️"],
  [/\b(?:trivial|minor|insignificant)\b/u, "🐜"],
  [/\b(?:not a big deal|no big deal)\b/u, "🤷"],
  [/\bheres the deal\b|\bhere is the deal\b/u, "☝️"],
  [/\bfood poisoning\b/u, "🤢"],
  [/\bstomach bug\b/u, "🦠"],
  [/\bfirst aid kit\b/u, "⛑️"],
  [/\bside effect\b/u, "⚠️"],
  [/\bultrasound diagnos(?:tic|is)\b/u, "📡"],
